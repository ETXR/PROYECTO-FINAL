var bj = { // propiedades y variables
  hdstand : null,  // dealer se queda
  hdpoints : null, // puntos del dealer
  hdhand : null,   // mano del dealer
  hpstand : null,  // jugador se queda
  hppoints : null, // puntos jugador
  hphand : null,   // jugador se queda
  hpcon : null,    // controles de jugador
  ...
  // (D) dibujamos los simbolos de cartas por medio de los de html
  dsymbols : ["&hearts;", "&diams;", "&clubs;", "&spades;"], // HTML symbols for cards
  dnum : { 1 : "A", 11 : "J", 12 : "Q", 13 : "K" }, // numerods de cartas
  draw : () => {
  ...
  check : () => {
    ...
    if (bj.player.length==2 && bj.dealer.length==2) {
      ...
      if (bj.ppoints==21 && bj.dpoints==21) {
        winner = 2; message = "EMPATE";
      }
      // PLAYER WINS
      if (winner==null && bj.ppoints==21) {
        winner = 0; message = "BLACKJACK!";
      }
      // DEALER WINS
      if (winner==null && bj.dpoints==21) {
        winner = 1; message = "DEALER BLACKJACK!";
      }
    }
    ...
    if (winner == null) {
      // PLAYER GONE BUST
      if (bj.ppoints>21) {
        winner = 1; message = "Te pasaste el dealer gana!";
      }
      // DEALER GONE BUST
      if (bj.dpoints>21) {
        winner = 0; message = "El dealer se pasa tu ganas!";
      }
    }
    ...
    if (winner == null && bj.dstand && bj.pstand) {
      ...
      if (bj.dpoints > bj.ppoints) {
        winner = 1; message = "Dealer gana con " + bj.dpoints + " puntos!";
      }
      // PLAYER HAS MORE POINTS
      else if (bj.dpoints < bj.ppoints) {
        winner = 0; message = "Tu ganas con " + bj.ppoints + " puntos!";
      }
      // TIE
      else {
        winner = 2; message = "Es un empate.";
      }
    }
